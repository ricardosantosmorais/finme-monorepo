...
namespace Finme.Application.Admin.Features.Account.Commands
{
    public class AccountBankCommandHandler(
        ...
        IMapper mapper) : IRequestHandler<AccountBankCommand, UserDto>
    {
        private readonly IRepository<User> _userRepository = userRepository;
        private readonly IRepository<UserBank> _userBankRepository = userBankRepository;
        private readonly IMapper _mapper = mapper;

        public async Task<UserDto> Handle(AccountBankCommand request, CancellationToken cancellationToken)
        {
